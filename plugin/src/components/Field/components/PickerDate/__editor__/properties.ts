...
let a: Editor.BaseProperties = {
  ...
  format: {
    ...
    desc: '',
    demo: `
    <p class="g-js-row">默认值为 <span class="g-js-block">YYYY-MM-DD</span></p>   
    格式说明: 
  ...
  disabledDate: {
    ...
    demo: `
    例如设置今天以前的时间不可选: 
    <p class="g-js-block"> <%:= current => current && current < moment().endOf("day") %> </p>
    <p>1. <span class="g-js-block">current</span> 是 <a href="http://momentjs.cn/" target="_blank"><span class="g-js-block">moment</span></a>对象</p>
    <p>2. 函数需要返回 <span class="g-js-block">true/false</span></p>
    `,
    type: 'string',
  },
}
